...
def write_report(ret, num_of_buckets, starttime, tally, counter, time_final, total_len, cycles, report_file, ret_minn, ret_maxx):
...
def write_distribution(reports_lib, num_of_buckets, file_quarter):
	with open("DRUG%s_report_lib.txt" % file_quarter, 'w', newline='') as csvfile:
		spamwriter = csv.writer(csvfile, delimiter=',')
...
ret_maxx = 	35							#Sets up higher limit - set to < 0 to create automatially
total = 0							#specifiy the number of files to create;  if set to 0, will go for the full list. 
iter_updates = 10000				#specifies how often the tally should be updated... need to also set to 0 to run full list.
iter_tracker = 0						#not sure I need this.... 
counter = 0								#tracks position in overall process, so updates can be provided in near real time. keep initalized at 0
key_tracker = []
reports_lib={}
reports_list = []
cycle_times = []
rand_rang = 4							#Set to 1/x to get % of files (e.g., set to 4 to get 25% of files, 5 to get 20%, 1, to get 100%
tally = 0								#Tally count to find out how many of total files were added. 
file_quarter = "17Q3"					#File quarter for each 

save_file = "DRUG%s_compiled.csv" % file_quarter
report_file = "DRUG%s_report.csv" % file_quarter

drug_ae_files = 'C:/Users/Andrew/Documents/Python_Scripts/FDA_Scripts/FAERS/faers_asci_2017q3/ascii/DRUG%s.txt' % file_quarter
demo_ae_files = 'C:/Users/Andrew/Documents/Python_Scripts/FDA_Scripts/FAERS/faers_asci_2017q3/ascii/DEMO%s.txt' % file_quarter
...
write_report(ret, num_of_buckets, starttime, tally, counter, time_final, total_len, cycles, report_file, ret_minn, ret_maxx)

write_distribution(reports_lib, num_of_buckets, file_quarter)
